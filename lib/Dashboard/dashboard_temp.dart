...
class _TeenPayDashboardState extends State<TeenPayDashboard> {
  ...
  Future<void> _fetchUserDetails() async {
    setState(() {
      displayName = 'Loading...'; // reset before fetching
    });
    ...
    try {
      ...
      // Step 2: Fetch UID from 'usernames' collection
      ...
      if (!usernameDoc.exists || usernameDoc.data() == null) {
        print('⚠️ No username document found for $loggedInUsername');
        setState(() {
          displayName = loggedInUsername; // fallback
        });
        return;
      }
      ...
      if (uid == null || uid.isEmpty) {
        print('⚠️ UID not found for username $loggedInUsername');
        setState(() {
      ...
      print('✅ Found UID for $loggedInUsername: $uid');

      // Step 3: Fetch KYC doc using UID
      ...
      if (!kycDoc.exists || kycDoc.data() == null) {
        print('⚠️ No KYC data found for UID $uid');
        setState(() {
          displayName = loggedInUsername;
        });
        return;
      ...
      // Step 4: Extract name field safely
      final kycData = kycDoc.data()!;
      final String? name =
          kycData['name'] ?? kycData['fullName'] ?? kycData['displayName'];

      print('✅ KYC name for $loggedInUsername: $name');

      setState(() {
        displayName = (name != null && name.isNotEmpty)
            ? name
            : loggedInUsername; // fallback to username if name missing
      });

      // Step 5: Fetch balance (optional)
              ...
              onTap: () {
                if (widget.username != null) {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => PayToFriendPage(
                        currentUid: widget.username!, // pass current username
                      ),
                    ),
                  );
                }
